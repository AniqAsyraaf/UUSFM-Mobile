...
import 'package:uusfm/service/image_converter.dart';
import 'package:google_ml_kit/google_ml_kit.dart';
import 'package:tflite_flutter/tflite_flutter.dart';
import 'package:image/image.dart' as imglib;

class FaceNetServiceCopy {
  // singleton boilerplate
  static final FaceNetServiceCopy _faceNetService =
      FaceNetServiceCopy._internal();

  factory FaceNetServiceCopy() {
  ...
  setCurrentPrediction(File cameraImage, Face face) {
  ...
  /// [face]: face detected
  List _preProcess(File image, Face faceDetected) {
    // crops the face 💇
  ...
  /// [cameraImage]: current image
  /// [face]: face detected
  imglib.Image _cropFace(File image, Face faceDetected) {
    //imglib.Image convertedImage = _convertCameraImage(image);
    imglib.Image originalImage =
        imglib.decodeImage(File(image.path).readAsBytesSync());
    double x = faceDetected.boundingBox.left - 10.0;
    double y = faceDetected.boundingBox.top - 10.0;
    double w = faceDetected.boundingBox.width + 10.0;
    double h = faceDetected.boundingBox.height + 10.0;
    return imglib.copyCrop(
        originalImage, x.round(), y.round(), w.round(), h.round());
  }
  ...
  String _searchResult(List predictedData) {
    ...
    print('sencond' + predictedData.toString());
    print('from database  : ' + data.toString());
